...
- **`tier` on `scf_get_assessment_review_queue` (#236).** `window` (default) lists window verdicts — the web app's Awaiting-confirmation queue now that the platform's windowed assessment flags default on — and `file` lists per-file verdicts. Entries carry `kind`.
- **System-type drift guard (#235).** `src/lib/system-types.ts` is the one source for the `system_type` vocabulary; `scripts/api-coverage.mjs` now fails when the platform's `SystemCreate.system_type` pattern differs from it, and `tests/system-types.test.ts` checks the enum, both tool schemas and the prose against the pattern captured in `tests/fixtures/platform-system-type-pattern.json`.

### Changed
- **`scf_get_assessment_review_queue` lists the window tier by default (#236).** Pass `tier: "file"` for the previous per-file listing.
