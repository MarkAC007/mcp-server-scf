...
- **codeql-action taken at v4.38.1, not #238's v4.38.0.** #238 was written on 16 Sep; Dependabot has since moved #230 on. Both SHAs were dereferenced against the upstream annotated tags before choosing (`b96794f…` really is v4.38.0, `1c5b675…` really is v4.38.1), and all four call sites move together — splitting them is what made the bot's original per-action PRs fail CodeQL with a version mismatch.
- **Two devDependency ranges tightened beyond what the bot proposed.** `lint-staged` `^17.0.5` → `^17.5.1` and `typescript-eslint` `^8.58.2` → `^8.70.0` come from #238's author, not from Dependabot — the bot left both ranges alone because the existing caret already admitted the new version, and bumped only the lockfile. Harmless (both are dev-only and the range now states what is actually installed), but it is a manual edit riding along with bot changes, so it is called out rather than buried.
- **`dependabot.yml` gains a `vitest` group and a `typescript` version ignore.** The group stops a vitest major from ever arriving as two un-installable PRs again. The ignore is temporary and carries its own removal condition: `typescript-eslint@8.70.0` declares peer `typescript ">=4.8.4 <6.1.0"`, so typescript 7 (#226) cannot install; drop the entry once that peer range widens.
- **The `typescript` ignore is pinned to the peer bound, not to `semver-major` as #238 wrote it.** #238's entry ignored only `version-update:semver-major`, which stops 7.x but waves through **6.1.0 — a minor** — straight into the `dev-dependencies` group, where it would fail `npm ci` in exactly the way #226 did. The condition is now `versions: [">=6.1.0"]`, which is the bound `typescript-eslint` actually declares.
