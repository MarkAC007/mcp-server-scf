...
**129 tools** across 11 domains — click through for full parameter tables and example prompts:
...
| [Organization](docs/tools/organization.md)                     | 8     | Users, orgs, audit trail, change cursor, work queue, notifications                                               |
...
Inspector opens on `http://localhost:6274` and connects to `mcp-server-scf` over stdio. You'll see all 129 tools, grouped by domain, with their Zod schemas rendered as a live form.
...
4. Claude Desktop restarts the server and all 129 tools are available.
